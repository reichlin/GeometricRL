...
parser.add_argument('--model_type', default=1, type=int)  # 0: GeomRL, 1: DQN
...
if args.model_type == 0:
    name_exp = "GeomRL_" + name_exp + "_z_dim="+str(dim_z)+"_reg="+str(reg)+"_use_policy="+str(use_policy)
else:
...
if args.model_type == 0:
    model = GeometricRL(dim_input, dim_input, dim_z, all_actions.shape[0], gamma, memory_size=memory_size, all_actions_idx=all_actions_idx, all_actions=all_actions, reg=reg, policy_type=0, device=device).to(device)
else:
    model = DQN(dim_input, all_actions.shape[0], gamma, memory_size, tau, n_layers=n_layers, device=device).to(device)
...
for epoch in tqdm(range(EPOCHS)):
    ...
    if epoch % sim_frq == sim_frq-1:
        ...
        np.savez("./saved_results_complexity/" + name_exp + ".npz", np.array(saved_rewards_test))
