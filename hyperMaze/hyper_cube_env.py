...
class HyperCube:
    ...
    def reset(self, random_reset=False):
        ...
        obs = {'observation': self.current_state, 'obstacles': self.get_surrounding(self.current_state), 'desired_goal': self.goal}
        return obs, None
    ...
    def step(self, action, state=None):
        ...
        if (not np.any(next_state < 0)) and (not np.any(next_state > 1-1e-6)):
            if self.obstacles is not None:
                ...
                self.current_state = next_state
        next_obs = {'observation': self.current_state, 'obstacles': self.get_surrounding(self.current_state), 'desired_goal': self.goal}
        reward = 0.0
        term = False
