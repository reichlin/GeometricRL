...
if use_images == 0:
    ...
    exploration_rewards = [50, 45, 40, 35, 30, 25, 20, 15, 10, 5, 1]
...
all_results_sparse = np.ones((len(exploration_rewards), len(models_trained), len(seeds), 100)) * -51
all_results_dense = np.ones((len(exploration_rewards), len(models_trained), len(seeds), 100)) * -51 # env, expl, algo, seed, time

folder_name = "./saved_results/" + environment_details["name"]
if use_images == 1:
    folder_name += "_img=1"
folder_name += "/"
    ...
    for j, algo_id in enumerate(models_trained):
        for k, seed in enumerate(seeds):
            ...
            if algo_id == 0:
                ...
                tmp_res_s = np.ones((2, 100)) * -51
                tmp_res_d = np.ones((2, 100)) * -51
